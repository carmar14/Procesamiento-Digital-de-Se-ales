...
[Audio fs]=audioread('G:\Laura\Proyecto final\Xantos - Bailame Despacio.mp3'); %%lee el sonido y retorna la señal en funcion del tiempo y la frecuencia de muestro fds
L=length(Audio);%%retorna el tamaño del vector audio( son numeros como un arreglo)
nfourier=2^nextpow2(L);%% haya el ultimo numero en base 2 para hallar el ancho del eje x
AudioF=fft(Audio,nfourier)/L;%% hace la transformada rapida de fourier a la señal audio
...
AuMeAl=filter(MediasAltas,Audio); %% creo una variable donde voy a meter la señal a la cual le voy a aplicar el filtro
